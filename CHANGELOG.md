...
### Fixed
- Hold mode having unintended toggle behavior on quick taps (<200ms)
- Default shortcut mismatch in validate() and tests (Option+R → Option+Space)

### Changed
- Redesign Settings to match Apple System Settings layout (NavigationSplitView sidebar, grouped forms)
- Redesign Onboarding with native grouped Form matching Settings style
- Extract Theme design tokens into centralized Theme.swift (spacing, typography, colors, radii, animations)
- Show max duration and silence timeout settings only in Toggle mode
- Default shortcut changed from Option+R to Option+Space

### Added
- `make clean-all` target for full fresh-run cleanup (models, config, caches, build artifacts)
